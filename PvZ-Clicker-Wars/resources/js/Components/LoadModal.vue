...
function formatDate(timestamp: number | null): string {
  if (!timestamp) return 'Nog nooit gesaved';
  ...
  <div class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-gray-900 border-2 border-cyan-500/60 rounded-xl p-6 max-w-md w-full shadow-2xl text-cyan-100 flex flex-col space-y-5 animate-in fade-in zoom-in duration-200">
      ...
      <p class="text-xs text-gray-300">
        Kies een save slot of de autosave om te laden:
      </p>
      ...
      <div class="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
        ...
        >
          ...
          <div class="flex justify-between items-center font-bold text-purple-300 group-hover:text-purple-200">
            <span>AUTOSAVE SLOT (Alleen laden)</span>
            <span class="text-xs text-purple-300 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-700/50">Laden</span>
          </div>
          <span class="text-xs text-gray-400">
            Laatst gesaved: <strong class="text-gray-300">{{ formatDate(slotMetadata.autosave.updatedAt) }}</strong>
          </span>
          <span v-if="!slotMetadata.autosave.exists" class="text-[10px] text-red-400 italic">Geen autosave beschikbaar</span>
        </button>
        ...
        >
          ...
          <div class="flex justify-between items-center font-bold text-cyan-200 group-hover:text-cyan-100">
            ...
            <span class="text-xs text-amber-400 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-700/50">Laden</span>
          </div>
          <span class="text-xs text-gray-400">
            Laatst gesaved: <strong class="text-gray-300">{{ formatDate(slotMetadata.A.updatedAt) }}</strong>
          </span>
          <span v-if="!slotMetadata.A.exists" class="text-[10px] text-red-400 italic">Slot is leeg</span>
        </button>
        ...
        >
          ...
          <span class="text-xs text-gray-400">
            Laatst gesaved: <strong class="text-gray-300">{{ formatDate(slotMetadata.B.updatedAt) }}</strong>
          </span>
          <span v-if="!slotMetadata.B.exists" class="text-[10px] text-red-400 italic">Slot is leeg</span>
        </button>
        ...
        >
          ...
          <span class="text-xs text-gray-400">
            Laatst gesaved: <strong class="text-gray-300">{{ formatDate(slotMetadata.C.updatedAt) }}</strong>
          </span>
          <span v-if="!slotMetadata.C.exists" class="text-[10px] text-red-400 italic">Slot is leeg</span>
        </button>
